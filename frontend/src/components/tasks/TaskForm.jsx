  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (groupsRes.data.length > 0 && !isEditing) {
          setFormData(prev => ({ ...prev, group: groupsRes.data[0].id }));
        }
        ...
        {groups.length === 0 && !isEditing ? (
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="grid-cards mb-6" style={{gridTemplateColumns: '1fr 1fr', gap: '1.5rem'}}>
              ...
              {user?.is_admin && (
                <div className="form-group mb-0">
                  <label className="form-label">Priority (Admin Only)</label>
                  <select 
                    className="form-input"
            ...
            {user?.is_admin && (
              <div className="form-group mb-6 flex items-center gap-3 p-4 rounded-lg" style={{background: 'var(--bg-secondary)', border: '1px solid var(--border-color)'}}>
                <input
                  type="checkbox"
                  id="is_enabled"
                ...
                <label htmlFor="is_enabled" className="form-label mb-0" style={{cursor: 'pointer'}}>
                  <strong>Task Enabled</strong> (Admin Only: Turn off to temporarily disable/hide this task)
                </label>
              </div>
            )}
